...
`featureRoot` の `features` を `lib` に置換したパスが対象。`.type.ts` は除外される。

| ディレクトリ | パターン   | Good              | Bad             |
| ------------ | ---------- | ------------------ | ----------------- |
| `lib/`       | `*.lib.ts` | `supabase.lib.ts`  | `supabase.ts`     |
|              |            | `stripe.lib.ts`    | `stripe.client.ts`|
|              |            | `redis.lib.ts`     | `redis.ts`        |

> `.type.ts` は命名規則の対象外（例: `supabase.type.ts` は許可される）

##### トップレベル utils ディレクトリ

`featureRoot` の `features` を `utils` に置換したパスが対象。`.type.ts` は除外される。

| 環境    | ディレクトリ                   | パターン    | Good               | Bad              |
| ------- | ------------------------------ | ----------- | ------------------- | ------------------ |
| Next.js | `src/utils/`                   | `*.util.ts` | `format.util.ts`    | `format.ts`        |
|         |                                |             | `date.util.ts`      | `date.helper.ts`   |
|         |                                |             | `string.util.ts`    | `utils.ts`         |
| Node.js | `scripts/utils/`               | `*.util.ts` | `parse.util.ts`     | `parse.ts`         |
|         |                                |             | `file.util.ts`      | `file.helper.ts`   |
| Deno    | `supabase/functions/_utils/`   | `*.util.ts` | `response.util.ts`  | `response.ts`      |
|         |                                |             | `validation.util.ts`| `validation.ts`    |

> `.type.ts` は命名規則の対象外（例: `response.type.ts` は許可される）

#### feature ディレクトリ名(`local/feature-name`)

- **Description**: `features/` 直下のディレクトリ名を `shared`、`auth`、および Supabase テーブル名(snake_case → kebab-case)に制限する。`supabase.type.ts` が存在しない場合は `shared` と `auth` のみ許可
- **Path**: `{featureRoot}/**/*.ts`
- **Bad**:

```ts
// features/my-stuff/services/server.service.ts — テーブルに存在しない名前
export function getStuff() {}
```

```ts
// features/comic_books/services/server.service.ts — snake_case は不可(kebab-case に変換が必要)
...
```ts
// features/comics/services/server.service.ts — テーブル名 "comics" に対応
export function getComics() {}
```

```ts
// features/shared/services/shared.service.ts — "shared" は常に許可
export function getConfig() {}
```

```ts
// features/auth/services/server.service.ts — "auth" は常に許可
export function getSession() {}
```

```ts
// features/comic-books/services/server.service.ts — テーブル名 "comic_books" → kebab-case
export function getComicBooks() {}
```
